...
async function getColorsFromExternal(): Promise<{
  primary: string;
  secondary: string;
}> {
  try {
    ...
    throw new Error(
      `Theme configuration unavailable: ${
...
export function createLightTheme(primaryColor: string, secondaryColor: string) {
  return {
    ...
    colors: {
      ...
      "surface-variant": adjustColor(secondaryColor, 0.4),
      "on-surface-variant": "#2D2D2D",

      // Text colors - ensuring high contrast
      "on-primary": getContrastColor(primaryColor),
      "on-secondary": "#FFFFFF", // Force white text on secondary (tan/brown) background
      "on-background": "#1C1D1A",
      "on-surface": "#1C1D1A", // Dark text on white surface
