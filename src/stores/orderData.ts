...
export const useOrderDataStore = defineStore("orderData", () => {
  ...
  /**
 * Fetch all pending orders for cashier review
 */
const fetchPendingOrders = async (): Promise<OrderWithMeals[]> => {
  try {
    loading.value = true;
    error.value = null;
    ...
    // Fetch order items with meal details for all pending orders
    const orderIds = ordersData.map((order) => order.id);
    ...
    const ordersWithMeals: OrderWithMeals[] = ordersData.map((order) => {
...
/**
 * Approve order and move to confirmed status (ready for kitchen)
 */
const approveOrder = async (orderId: number): Promise<Order | null> => {
  try {
    loading.value = true;
    error.value = null;
 ...
 * Reject/Cancel order
 */
const rejectOrder = async (orderId: number): Promise<Order | null> => {
  try {
    loading.value = true;
    error.value = null;
 ...
 */
const getOrderSummary = (orderWithMeals: OrderWithMeals): {
  items: { meal: MenuItem; quantity: number; subtotal: number }[];
  total: number;
} => {
  if (!orderWithMeals.order_items_db || orderWithMeals.order_items_db.length === 0) {
    return { items: [], total: 0 };
  }

  const items = orderWithMeals.order_items_db.map((item) => ({
    meal: item.meal,
